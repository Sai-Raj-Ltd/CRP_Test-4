...
class SendMultiPayslips(models.TransientModel):
    ...
    def multi_send_by_email(self):
        """This method will send a list of selected payslips all in one."""
        for slip in self.env[self._context.get('active_model')].browse(
                self._context.get('active_ids')):
            ctx = {}
            ctx.update(self._context)
            ctx.update(slip.send_by_email().get('context'))
            slip.with_context(ctx).message_post_with_template(
